...
### 修复
- 🔧 深度修复用户删除日记后关联图片文件未清理的问题
  - 问题原因：
    1. `deleteDiary` 函数只删除数据库中的日记记录，未清理关联的图片文件
    2. `updateDiary` 函数在用户移除图片时，未删除旧的图片文件
    3. 导致 `uploads` 目录中积累大量未使用的图片文件，占用磁盘空间
  - 修复方案：
    1. **deleteDiary 函数**（第350-380行）：
       - 删除日记前先获取关联的图片列表
       - 删除数据库记录后遍历图片列表，删除所有关联文件
       - 使用正确的路径处理：提取 `/uploads/filename.ext` 中的文件名，构建完整路径
    2. **updateDiary 函数**（第280-310行）：
       - 比较新旧图片列表，找出被移除的图片
       - 更新日记后删除不再使用的图片文件
       - 同样的路径处理逻辑确保文件正确删除
    3. **错误处理**：
       - 图片删除失败不影响主操作，只记录错误日志
       - 使用 `fs.existsSync` 检查文件存在性后再删除
  - 修改的文件：
    - `backend/src/controllers/diaryController.js`

### 验证
- ✅ 更新日记移除图片时，对应图片文件被正确删除
- ✅ 删除日记时，所有关联图片文件被正确删除
- ✅ 测试通过率：预期删除 3 个文件，实际删除 3 个文件

### 技术细节
- 路径处理：正确处理 `/uploads/` 前缀，提取文件名后构建完整路径
- 安全性：使用 `fs.existsSync` 检查文件存在性后再删除
- 容错性：图片删除失败不影响主操作，只记录错误日志
